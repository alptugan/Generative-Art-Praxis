(function (root, factory) {
    ...
    /**
     * @description Returns an array of object that holds position and size information for each grid element. Fills the grid row by row. 
     * @method createGrid
     * @param {number} _row Number of rows
     * @param {number} _col Number of columns
     * @param {number} _totalW The width of total grid space 
     * @param {number} _totalH The height of total grid space 
     * @param {number} [_margin=0] Gap between each grid element
     * @param {boolean} [_alignX = false] Alignment on the x-axis according to the width of the canvas
     * @param {boolean} [_alignY = false] Alignment on the y-axis according to the height of the canvas
     * @returns {Object.<number>}
     */
    p5.Utils.prototype.createGrid = function (_row, _col, _totalW, _totalH, _margin = 0, _type = GRID.SQUARE, _alignX = true, _alignY = true) {
        var att = [];
        var xShift = 0;
        var yShift = 0;
        var sz = _row > _col ? max(_totalW, _totalH) : min(_totalW, _totalH);

        var rt = max(_row, _col);

        sz -= _margin * (rt);

        var _w = _type == GRID.SQUARE ? sz / rt : sz / _col;
        var _h = _type == GRID.SQUARE ? sz / rt : sz / _row;

        var totalW = (_w + _margin) * _col - _margin;
        var totalH = (_h + _margin) * _row - _margin;

        var boundsRatio = _totalW / _totalH;
        var gridRatio = totalW / totalH;

        // ref: http://www.frontcoded.com/javascript-fit-rectange-into-bounds.html
        if (gridRatio > boundsRatio) {
            // landscape
            _w = Math.floor(_totalW / _col) - _margin;
            _h = Math.floor(_totalW / _col) - _margin;

        } else {
            // portrait
            _w = Math.floor(_totalH / _row) - _margin;
            _h = Math.floor(_totalH / _row) - _margin;
        }

        var totalW = (_w + _margin) * _col - _margin;
        var totalH = (_h + _margin) * _row - _margin;

        xShift = _alignX ? Math.floor((width - totalW) * 0.5) : 0;
        yShift = _alignY ? Math.floor((height - totalH) * 0.5) : 0;

        for (var i = 0; i < _row; i++) {
            for (var j = 0; j < _col; j++) {

                att.push({
                    "x": xShift + Math.floor(j * (_w + _margin)),
                    "y": yShift + Math.floor(i * (_h + _margin)),
                    "w": _w,
                    "h": _h
                });
            }
